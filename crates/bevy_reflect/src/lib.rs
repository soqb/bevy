...
pub mod prelude {
    ...
    #[doc(hidden)]
    pub use crate::{
        reflect_trait, FromReflect, GetField, GetTupleStructField, PartialReflect,
        ReflectDeserialize, ReflectSerialize, Struct, TupleStruct,
    };
}
            ...
            let data = r#"
{
    "glam::f32::vec3::Vec3": (
        ...
        fn vec3_path_access() {
            ...
            assert_eq!(
                *v.path("x").unwrap().try_downcast_ref::<f32>().unwrap(),
                1.0
            );

            *v.path_mut("y").unwrap().try_downcast_mut::<f32>().unwrap() = 6.0;
